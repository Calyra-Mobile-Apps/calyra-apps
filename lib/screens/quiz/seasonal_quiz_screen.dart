...
class SeasonalColorQuizScreen extends StatefulWidget {
  ...
  @override
  State<SeasonalColorQuizScreen> createState() =>
      _SeasonalColorQuizScreenState();
}
...
class _SeasonalColorQuizScreenState extends State<SeasonalColorQuizScreen> {
  ...
  void _initializeBackgroundColor() {
    ...
    if (_currentStep < palettes.length &&
        palettes[_currentStep].containsKey(season1Name)) {
      setState(() {
  ...
  void _onNextPressed() {
    if (_selectedSeasonForRow == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Pilih salah satu baris palet terlebih dahulu')),
      );
      return;
    }
    final isWarm =
        context.read<QuizProvider>().answers[QuizKeys.undertone] == 'warm';
    final season1Name = isWarm ? 'Spring' : 'Summer';
    ...
    final totalSteps =
        isWarm ? warmStepPalettes.length : coolStepPalettes.length;
    if (_currentStep < totalSteps - 1) {
      setState(() {
        ...
        _activeSelfieBackgroundColor =
            palettes[_currentStep][nextSeason1Name]![0];
      });
    } else {
          ...
          child: Column(
            children: [
              ...
              _ColorPaletteRow(
                ...
                onSelect: () => setState(() {
                  _selectedSeasonForRow = season1Name;
                  _activeSelfieBackgroundColor = season1Colors.first;
                }),
              ),
              const SizedBox(height: 24),
              _ColorPaletteRow(
                ...
                onSelect: () => setState(() {
                  _selectedSeasonForRow = season2Name;
                  _activeSelfieBackgroundColor = season2Colors.first;
                }),
              ),
              const SizedBox(height: 32),
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: _selectedSeasonForRow != null
                        ? Colors.black
                        : Colors.grey[300],
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 18,
                      color: _selectedSeasonForRow != null
                          ? Colors.white
                          : Colors.grey[600],
                    ),
                  ),
